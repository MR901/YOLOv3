...
pipmain(['install', 'skimage'])
pipmain(['install', 'matplotlib==3.0.3'])
pipmain(['install', 'tkinter'])
pipmain(['install', 'imgaug==0.2.9'])
pipmain(['install', 'scipy==1.2.1'])
pipmain(['install', 'opencv_python'])
pipmain(['install', 'Keras'])
pipmain(['install', 'Pillow==6.0.0'])
pipmain(['install', 'h5py==2.9.0'])
pipmain(['install', 'requests==2.21.0'])
...
def detect_and_color_splash(model, image_path=None, video_path=None, out_dir=''):
    ...
    class_names = ['BG', 'blackcrumb', 'redrcumb']
    ...
    if image_path:
        ...
        # Read image
        image = skimage.io.imread(args.image, plugin='matplotlib')
        # Detect objects
